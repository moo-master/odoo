...
class AccountTax(models.Model):
    ...
    def compute_all(
            ...
            discount=0):
        """
            This is overide compute function
        ...
        def recompute_base(
                base_amount, fixed_amount, percent_amount, division_amount
        ):
        ...
        base = currency.round((price_unit * quantity) - discount)

        # Origin
        # base = currency.round(price_unit * quantity)

        # For the computation of move lines, we could have a negative base value.
        # In this case, compute all with positive values and negate them at the
        # end.
        sign = 1
        ...
        # Store the totals to reach when using price_include taxes (only the
        # last price included in row)
        total_included_checkpoints = {}
        i = len(taxes) - 1
        store_included_tax_total = True
        # Keep track of the accumulated included fixed/percent amount.
        incl_fixed_amount = incl_percent_amount = incl_division_amount = 0
        # Store the tax amounts we compute while searching for the
        # total_excluded
        cached_tax_amounts = {}
            ...
            for tax in reversed(taxes):
                ...
                if tax.price_include or self._context.get(
                    ...
                    if tax.amount_type == 'percent':
                        ...
                        incl_fixed_amount += abs(quantity) * \
                            tax.amount * sum_repartition_factor
                    else:
        ...
        # Start the computation of accumulated amounts at the total_excluded
        # value.
        ...
        # the tax_tag_ids of all the move lines created by the compute all for
        # this product.
        ...
        for tax in taxes:
            ...
            if not skip_checkpoint and price_include and total_included_checkpoints.get(
                ...
                # We know the total to reach for that tax, so we make a
                # substraction to avoid any rounding issues
            ...
            # Round the tax_amount multiplied by the computed repartition lines
            # factor.
            tax_amount = float_round(tax_amount, precision_rounding=prec)
            factorized_tax_amount = float_round(
                tax_amount * sum_repartition_factor,
            ...
            repartition_line_amounts = [
                float_round(
                    ...
                    precision_rounding=prec) for line in tax_repartition_lines]
            ...
            rounding_error = float_round(
                nber_rounding_steps and total_rounding_error
                / nber_rounding_steps or 0.0,
                precision_rounding=prec)
    ...
    def _compute_amount(
        ...
        res = super(
            ...
            product=None,
            partner=None)
        if self.amount_type not in ['fixed', 'percent', 'division']:
            return 1
        return res
