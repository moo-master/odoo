...
class Budget(models.Model):
    ...
    name = fields.Char(
        'Budget Name', required=True, states={
            'done': [
                ('readonly', True)]})
    ...
    date_from = fields.Date(
        'Start Date', required=True, states={
            'done': [
                ('readonly', True)]})
    date_to = fields.Date(
        'End Date', required=True, states={
            'done': [
                ('readonly', True)]})
    state = fields.Selection([('draft',
                             ...
                             tracking=True)
    budget_line = fields.One2many(
        'budget.lines', 'budget_id', 'Budget Lines', states={
            'done': [
    ...
    def _compute_theoretical_amount(self):
        ...
        for line in self:
            ...
            if self.env.context.get(
                    'wizard_date_from') and self.env.context.get('wizard_date_to'):
