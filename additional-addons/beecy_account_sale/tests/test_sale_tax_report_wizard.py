import pytest
from pytest_tr_odoo.fixtures import env
from unittest.mock import call
from odoo import fields
from odoo.exceptions import ValidationError
import xlsxwriter
from io import BytesIO


@pytest.fixture
def model(env):
    return env['sale.tax.report.wizard']


@pytest.fixture
def account_move(env):
    return env['account.move']


@pytest.fixture
def product1(env):
    product = env.ref('product.product_order_01')
    return product


@pytest.fixture
def product2(env):
    product = env.ref('product.product_product_3')
    return product


@pytest.fixture
def partner_demo(env):
    partner = env.ref('base.partner_demo')
    return partner


@pytest.fixture
def partner_1(env):
    partner = env.ref('base.res_partner_1')
    return partner


@pytest.fixture
def payment_term_30days(env):
    payment_term = env.ref('account.account_payment_term_30days')
    return payment_term


@pytest.fixture
def journal_sale(env):
    return env['account.journal'].search([
        ('type', '=', 'sale'),
        ('company_id', '=', env.company.id)], limit=1)


@pytest.mark.parametrize('test_input,expected', [
    ({'company': True, 'move_type': 'out_invoice'}, True),
    ({'company': False, 'move_type': 'out_refund'}, True),
])
def test_print_xls_report(
        env,
        model,
        account_move,
        partner_demo,
        payment_term_30days,
        journal_sale,
        product1,
        test_input,
        expected
):

    acc = account_move.create({
        'move_type': test_input.get('move_type'),
        'invoice_date': '2022-03-23',
        'company_id': env.company.id,
        'partner_id': partner_demo.id,
        'invoice_payment_term_id': payment_term_30days.id,
        'journal_id': journal_sale.id,
        'invoice_line_ids': [(0, 0, {
            'product_id': product1.id
        })],
    })
    acc.update({'state': 'posted'})
    sale_tax = model.create({
        'company_ids': env.company.ids,
        'start_date': '2022-03-01',
        'end_date': '2022-03-31'
    })
    if not test_input.get('company'):
        sale_tax.write({'company_ids': False})
    res = sale_tax.print_xls_report()
    assert res['name'] == 'Sale Tax Report'


@pytest.fixture
def sale_tax_wizard(env):
    return env['report.beecy_account_sale.sale_tax']


def report_xlsx(env):
    return env['report.report_xlsx.abstract']


@pytest.mark.parametrize('test_input,expected', [
    ({'move_type': 'out_invoice'}, True),
    ({'move_type': 'out_refund'}, True),
    ({'move_type': 'out_debit'}, True),
])
def test_generate_xlsx_report(env, sale_tax_wizard,
                              account_move,
                              partner_demo,
                              payment_term_30days,
                              journal_sale,
                              product1,
                              test_input,
                              expected):
    acc = account_move.create({
        'move_type': test_input.get('move_type'),
        'invoice_date': '2022-03-23',
        'company_id': env.company.id,
        'partner_id': partner_demo.id,
        'invoice_payment_term_id': payment_term_30days.id,
        'journal_id': journal_sale.id,
        'invoice_line_ids': [(0, 0, {
            'product_id': product1.id
        })],
    })
    acc.update({'state': 'posted'})
    account_move = env['account.move'].search(
        [
            ('move_type', 'in', ['out_invoice', 'out_refund', 'out_debit']),
            ('state', 'in', ['posted', 'cancel']),
            ('invoice_date', '>=', '2022-03-01'),
            ('invoice_date', '<=', '2022-03-31')
        ])
    file_data = BytesIO()
    wizard = env['sale.tax.report.wizard'].create({
        'start_date': '2022-03-01',
        'account_move_ids': [(6, 0, account_move.ids)]
    })
    workbook = xlsxwriter.Workbook(file_data, {})
    data = False
    sale_tax_wizard.generate_xlsx_report(workbook, data, wizard)
