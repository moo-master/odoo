...
class AccountReconciliation(models.AbstractModel):
    ...
    def get_move_lines_for_bank_statement_line(
        ...
        from_clause, where_clause, where_clause_params = self.env['account.move.line']._where_calc(
            domain).get_sql()
        query_str = '''
    ...
    def _get_bank_statement_line_partners(self, st_lines):
        ...
        # identical bank accounts may exist in a company we don't have access
        # to.
    ...
    def get_bank_statement_line_data(self, st_line_ids, excluded_ids=None):
        ...
        for line in bank_statement_lines:
            if matching_amls[line.id].get('status') == 'reconciled':
                reconciled_move_lines = matching_amls[line.id].get(
                    'reconciled_lines')
                results['value_min'] += 1
    ...
    def get_data_for_manual_reconciliation(
        ...
        if res_ids is not None and len(res_ids) == 0:
            ...
            # since postgresql doesn't implement empty list (so 'AND id in ()'
            # is useless)
            return []
    ...
    def process_move_lines(self, data):
        ...
        for datum in data:
            if len(datum['mv_line_ids']) >= 1 or len(
                    datum['mv_line_ids']) + len(datum['new_mv_line_dicts']) >= 2:
    ...
    def _domain_move_lines_for_reconciliation(
        ...
        # Always exclude the journal items that have been marked as 'to be
        # checked' in a former bank statement reconciliation
        ...
        if mode == 'rp':
            domain = expression.AND([domain, [('account_id.internal_type', 'in', [
                                    'receivable', 'payable', 'liquidity'])]])
        else:
            domain = expression.AND([domain, [('account_id.internal_type', 'not in', [
                                    'receivable', 'payable', 'liquidity'])]])
    ...
    def _domain_move_lines_for_manual_reconciliation(
        ...
        # filter on account.move.line having the same company as the given
        # account
    ...
    def _prepare_move_lines(
        ...
        for line in move_lines:
            ...
            line_currency = (
                line.currency_id and line.amount_currency) and line.currency_id or company_currency
            ret_line = {
                'id': line.id,
            ...
            # For already reconciled lines, don't use
            # amount_residual(_currency)
            ...
            # the value of the amount_currency or the debit-credit if no amount
            # currency
            ...
            ret_line['amount_currency_str'] = amount_currency and formatLang(
                self.env, abs(amount_currency), currency_obj=line_currency) or ""
            ret_line['total_amount_currency_str'] = total_amount_currency and formatLang(
                self.env, abs(total_amount_currency), currency_obj=line_currency) or ""
            ret.append(ret_line)
        return ret
    ...
    def _process_move_lines(self, move_line_ids, new_mv_line_dicts):
        ...
        """
        if len(move_line_ids) < 1 or len(
                move_line_ids) + len(new_mv_line_dicts) < 2:
            raise UserError(
                _('A reconciliation must involve at least 2 move lines.'))
        ...
        if len(new_mv_line_dicts) > 0:
            ...
            same_currency = False
            currencies = list(
                set([aml.currency_id or company_currency for aml in account_move_line]))
        ...
        for journal_id, lines in writeoff_dict.items():
            ...
            for vals in lines:
                ...
                if vals['date'] and vals['date'] < date:
                    date = vals['date']
                if 'name' not in vals:
                    vals['name'] = self._context.get(
                        'comment') or _('Write-Off')
                ...
                if 'amount_currency' not in vals and writeoff_currency != company_currency:
                    ...
                    vals['amount_currency'] = sign * \
                        abs(sum([r.amount_residual_currency for r in self]))
            ...
            line_to_reconcile += writeoff_move.line_ids.filtered(
                lambda r: r.account_id == self[0].account_id).sorted(key='id')[-1:]
...
class AccountBankStatementLine(models.Model):
    ...
    def process_reconciliation(
        ...
        for aml_dict in counterpart_aml_dicts + new_aml_dicts:
            if aml_dict.get("tax_ids") and isinstance(
                    aml_dict["tax_ids"][0], int):
        ...
        if counterpart_aml_dicts or new_aml_dicts:
            ...
            aml_obj.with_context(
                check_move_validity=False).create(liquidity_aml_dict)
    ...
    def _prepare_move_line_for_currency(self, aml_dict, date):
        ...
        if st_line_currency.id != company_currency.id:
            ...
            # company currency
            prorata_factor = (
                aml_dict['debit'] - aml_dict['credit']) / self.amount_currency
