...
class AccountAssetAsset(models.Model):
    ...
    def compute_generated_entries(self, date, asset_type=None):
        # Entries generated : one by grouped category and one by asset from
        # ungrouped category
        created_move_ids = []
        type_domain = []
        if asset_type:
    ...
    def compute_depreciation_board(self):
        ...
        # Remove old unposted depreciation lines. We cannot use unlink() with
        # One2many field
        ...
        if self.value_residual != 0.0:
            ...
            if self.prorata:
                ...
                # date is last entry + method perio
                ...
                if self.method_period >= 12:
                    if self.company_id.fiscalyear_last_month:
                        asset_date = date(
                            year=int(
                                self.date.year), month=int(
                                self.company_id.fiscalyear_last_month), day=int(
                                self.company_id.fiscalyear_last_day)) + relativedelta(
                            days=1) + relativedelta(
                            year=int(
